...
/usr/bin/docker build -t="10.160.140.32:5000/letv_mcluster_gbalancer:${VERSION}" mcluster_vip_gbalancer/  >> ${BUILE_LOG}
echo ' -------------------start building letv-mcluster-gbalancer images----------------------' >> ${BUILE_LOG}

echo ' -------------------start push letv-mcluster images----------------------' >> ${BUILE_LOG}
/usr/bin/docker push 10.160.140.32:5000/letv-mcluster:${VERSION}  >> ${BUILE_LOG}
echo ' -------------------finish push letv-mcluster images----------------------' >> ${BUILE_LOG}

echo ' -------------------start push letv-mcluster-gbalancer images----------------------' >> ${BUILE_LOG}
/usr/bin/docker push 10.160.140.32:5000/letv_mcluster_gbalancer:${VERSION}  >> ${BUILE_LOG}
