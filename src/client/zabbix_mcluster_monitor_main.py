...
def check_monitor_main(serious_dict, general_dict, nothing_dict):
    ...
    for monitor_type in monitor_return_json_value['response']:
        for monitor_item in monitor_return_json_value['response'][monitor_type]:
            ...
            logic_return_code = monitor_return_json_value['response'][monitor_type][monitor_item]['alarm']
            logic_return_message = monitor_return_json_value['response'][monitor_type][monitor_item]['message']
            
            if logic_return_code == 'tel:sms:email':
                serious_dict.setdefault(monitor_type_item,logic_return_message)
            elif logic_return_code == 'sms:email':
                general_dict.setdefault(monitor_type_item,logic_return_message)
            else:
                nothing_dict.setdefault(monitor_type_item,logic_return_message)
