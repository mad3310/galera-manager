...
class Check_Status_Base(object):
    ...
    def check_status(self, data_node_info_list, url_post, monitor_type, monitor_key):
        ...
        self.write_status(zk_data_node_count, success_count, failed_count, alarm_level, error_record_dict, monitor_type,
                          monitor_key)
        ...
        if not conn:
            ...
            try:
                ...
                if anti_item_procedure_count and on_check_storedprocedure:
                    anti_item_message = "check db status, existed stored procedure. Item's count:%s" % (
...
class Check_Database_User(Check_Status_Base):
    ...
    def check(self, data_node_info_list):
        ...
        if len(user_zk_src_list) == 0 and len(user_mysql_src_dict) == 0:
            ...
            alarm_level = self.retrieve_alarm_level(count_dict_set["total"], count_dict_set["success"],
        ...
        for list_iter in _user_zk_src_list:
            if list_iter[0] in _user_mysql_src_dict_keys:
                if long(list_iter[1]["max_user_connections"]) == _user_mysql_src_dict[list_iter[0]][0] and \
                                long(list_iter[1]["max_connections_per_hour"]) == _user_mysql_src_dict[list_iter[0]][
                            1] and \
