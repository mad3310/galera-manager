...
# retrieve the node stat for mem available
# eg. curl "http://localhost:8888/node/stat/memory/available"
class StatMemAvailable(APIHandler):
    stat_opers = NodeStatOpers()

    def get(self):
        result = self.stat_opers.stat_data_mem_available()
        self.finish(result)


# retrieve the /srv/mcluster and /data directory disk available
# eg. curl "http://localhost:8888/node/stat/disk/available"
...
# retrieve the node stat for work load
# eg. curl "http://localhost:8888/node/stat/workload"
