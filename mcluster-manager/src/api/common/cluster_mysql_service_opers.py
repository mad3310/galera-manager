...
class PortStatus():
    def __init__(self):
        ...
        constructor
        """
    def check_port(self, c_data_node_info_list, c_start_node_ip_list):
        ...
        for data_node_ip in c_data_node_info_list:
            ...
            if return_request == 'false':
               c_start_node_ip_list.append(data_node_ip) 
...
class GaleraStatus():
    def __init__(self):
        ...
        constructor
        """
    def check_status(self,  c_data_node_stop_finished_flag_dict, c_uuid_seqno_dict):
        ...
        for (data_node_ip, stop_finished) in c_data_node_stop_finished_flag_dict.iteritems():
            if not stop_finished:
                error_message = "data node(%s) didn't stop finished, please check the status and start it by human." % (data_node_ip)
            ...
            c_uuid_seqno_dict.setdefault(data_node_ip, uuid_seqno_sub_dict)
...
class Cluster_start_action(Abstract_Mysql_Service_Action_Thread):
    ...
    def _issue_start_action(self, lock, cluster_flag):
        ...
        status_dict = {}
        status_dict.setdefault("_status", "initialing")
        self.zkOper.write_monitor_status("node" ,"cluster_status" , status_dict)
        ...
        try:
            if cluster_flag == 'new':
                ...
                p_num = portstatus_obj.check_port(data_node_info_list ,start_node_ip_list)
                try:
                     if p_num != 1:
                ...
                if stop_finished_count == len(data_node_stop_finished_flag_dict):
                    ...
                    self.zkOper.write_monitor_status("node", "cluster_status", status_dict)
                    self._send_email("mcluster", " mysql service stop operation finished on cluster")
                
                logging.info('Finish stopping nodes!')     
                
                uuid_seqno_dict = {}
                
                galerastatus_obj = GaleraStatus()
                galerastatus_obj.check_status(data_node_stop_finished_flag_dict, uuid_seqno_dict)
                
                start_node_ip_list = self._sort_seqno(uuid_seqno_dict)
                logging.info("After sort, the uuid_seqno_dict value is %s" % str(start_node_ip_list))
    
            url_post = "/node/start"
            logging.info("/node/start start issue!")
            for data_node_ip in start_node_ip_list:
                started_nodes = self.zkOper.retrieve_started_nodes()
                started_nodes_count = len(started_nodes)
            
                isNewCluster = False
                if started_nodes_count == 0:
                    isNewCluster = True
            ...
            logging.info("Unlock cluster start stop_action done!")
        ...
        try:
            ...
            logging.info("starts ondes" + str(nodes_online))       
            if nodes_online == node_num:
                status_dict['_status'] = 'running'
                self.zkOper.write_monitor_status("node", "cluster_status", status_dict)
                self._send_email("mcluster", " mysql service start operation finished on cluster")
        except Exception, e:
            logging.error(e)
    ...
    def _sort_seqno(self, dict):
        ...
        for (data_node_ip, uuid_seqno_sub_dict) in dict.items():
            ...
            if '00000000-0000-0000-0000-000000000000' == uuid:
                start_node_ip_list.append(data_node_ip)
                continue
        ...
        if 0: #len(uuid_unique_set) != 1:  #Maybe something wrong here.
        ...
        for seqno in seqno_list:
            for (data_node_ip, uuid_seqno_sub_dict) in dict.items():
                ...
                if seqno == seqno_target:
                    start_node_ip_list.insert(0,data_node_ip)
                    del dict[data_node_ip]
                    break
                    
        return start_node_ip_list
